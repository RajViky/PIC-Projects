...
unsigned short int basespeed = 190, Kp = 5, Ki = 8, Kd = 4;       // 5 2 2          5 3 4
 ...
 while(1)
 {
   ...
   CalcError();
    ...
    { for(i=0;i<20;i++)
     {
       lastreading = 'u';
       motor_RF();						          //TURN LEFT AT FULL SPEED
			 motor_LB();
       PWM1_CHANGE_DUTY(255);
       PWM2_CHANGE_DUTY(255);
       delay_ms(10);
    ...
    if(PORTB.F7 == 0 && (PORTE.F1==1 ||PORTA.F0==1))
    {
      for(i=0;i<10;i++)
     {
      motor_LF();					        	      //FWD AT FULL SPEED
