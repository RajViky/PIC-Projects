...
unsigned short int basespeed = 120, Kp = 90, Ki = 10, Kd = 35; // 90 10 20
...
void initport()
{
  ...
  TRISB = 0b11000000;    //INITIALIZE PORTB AS INPUT
}
...
void main()
{
 ...
 while(1)
 {
   ...
   if((s1+s2+s3+s4+s5+s6+s7+s8) == 0)		//ROBOT HAS OVERSHOOT
	 {
      if(PORTB.F7)                   // TSOP NOT DETECTING
	    {
          ...
          delay_ms(10);
			  ...
			  else(lastreading == 'l')			  //CHECKS IF THE LAST SENSOR ACTIVATED WAS LEFT
				{
    ...
    {
     if(PORTB.F7==0)                         // TSOP FRONT ACTIVE
     {
     ...
     else(PORTB.F7==1)                     //TSOP NOT ACTIVE
    	{
				PROPORTIONAL = error * kp;
        INTEGRAL += error ;
        INTEGRAL *= ki;
