;  ASM code generated by mikroVirtualMachine for PIC - V. 8.2.0.0
;  Date/Time: 2/4/2011 9:32:56 PM
;  Info: http://www.mikroe.com


; ADDRESS	OPCODE	ASM
; ----------------------------------------------
$0000	$2804			GOTO	_main
$0004	$	_main:
;portd7.c,2 :: 		void main()
;portd7.c,4 :: 		trisD=0X00;
$0004	$1303			BCF	STATUS, RP1
$0005	$1683			BSF	STATUS, RP0
$0006	$0188			CLRF	TRISD, 1
;portd7.c,5 :: 		for(i=0;i<75;i++)
$0007	$1283			BCF	STATUS, RP0
$0008	$01A0			CLRF	_i
$0009	$01A1			CLRF	_i+1
$000A	$	L_main_0:
$000A	$3000			MOVLW	0
$000B	$0221			SUBWF	_i+1, 0
$000C	$1D03			BTFSS	STATUS, Z
$000D	$2810			GOTO	L_main_9
$000E	$304B			MOVLW	75
$000F	$0220			SUBWF	_i, 0
$0010	$	L_main_9:
$0010	$1803			BTFSC	STATUS, C
$0011	$2836			GOTO	L_main_1
;portd7.c,7 :: 		PORTD.F7=1;        //SERVO_4
$0012	$1788			BSF	PORTD, 7
;portd7.c,8 :: 		delay_us(1500);
$0013	$3002			MOVLW	2
$0014	$00FB			MOVWF	STACK_11
$0015	$30FF			MOVLW	255
$0016	$00FA			MOVWF	STACK_10
$0017	$0BFB			DECFSZ	STACK_11, F
$0018	$281A			GOTO	$+2
$0019	$281D			GOTO	$+4
$001A	$0BFA			DECFSZ	STACK_10, F
$001B	$281A			GOTO	$-1
$001C	$2817			GOTO	$-5
$001D	$30F0			MOVLW	240
$001E	$00FA			MOVWF	STACK_10
$001F	$0BFA			DECFSZ	STACK_10, F
$0020	$281F			GOTO	$-1
$0021	$0000			NOP
$0022	$0000			NOP
;portd7.c,9 :: 		PORTD.F7=0;
$0023	$1388			BCF	PORTD, 7
;portd7.c,10 :: 		delay_us(18500);
$0024	$3018			MOVLW	24
$0025	$00FB			MOVWF	STACK_11
$0026	$30FF			MOVLW	255
$0027	$00FA			MOVWF	STACK_10
$0028	$0BFB			DECFSZ	STACK_11, F
$0029	$282B			GOTO	$+2
$002A	$282E			GOTO	$+4
$002B	$0BFA			DECFSZ	STACK_10, F
$002C	$282B			GOTO	$-1
$002D	$2828			GOTO	$-5
$002E	$30F6			MOVLW	246
$002F	$00FA			MOVWF	STACK_10
$0030	$0BFA			DECFSZ	STACK_10, F
$0031	$2830			GOTO	$-1
;portd7.c,11 :: 		}
$0032	$	L_main_2:
;portd7.c,5 :: 		for(i=0;i<75;i++)
$0032	$0AA0			INCF	_i, 1
$0033	$1903			BTFSC	STATUS, Z
$0034	$0AA1			INCF	_i+1, 1
;portd7.c,11 :: 		}
$0035	$280A			GOTO	L_main_0
$0036	$	L_main_1:
;portd7.c,12 :: 		for(i=0;i<75;i++)
$0036	$01A0			CLRF	_i
$0037	$01A1			CLRF	_i+1
$0038	$	L_main_3:
$0038	$3000			MOVLW	0
$0039	$0221			SUBWF	_i+1, 0
$003A	$1D03			BTFSS	STATUS, Z
$003B	$283E			GOTO	L_main_10
$003C	$304B			MOVLW	75
$003D	$0220			SUBWF	_i, 0
$003E	$	L_main_10:
$003E	$1803			BTFSC	STATUS, C
$003F	$2863			GOTO	L_main_4
;portd7.c,14 :: 		PORTD.F7=1;        //SERVO_6
$0040	$1788			BSF	PORTD, 7
;portd7.c,15 :: 		delay_us(2100);
$0041	$3003			MOVLW	3
$0042	$00FB			MOVWF	STACK_11
$0043	$30FF			MOVLW	255
$0044	$00FA			MOVWF	STACK_10
$0045	$0BFB			DECFSZ	STACK_11, F
$0046	$2848			GOTO	$+2
$0047	$284B			GOTO	$+4
$0048	$0BFA			DECFSZ	STACK_10, F
$0049	$2848			GOTO	$-1
$004A	$2845			GOTO	$-5
$004B	$30B7			MOVLW	183
$004C	$00FA			MOVWF	STACK_10
$004D	$0BFA			DECFSZ	STACK_10, F
$004E	$284D			GOTO	$-1
$004F	$0000			NOP
;portd7.c,16 :: 		PORTD.F7=0;
$0050	$1388			BCF	PORTD, 7
;portd7.c,17 :: 		delay_us(17900);
$0051	$3018			MOVLW	24
$0052	$00FB			MOVWF	STACK_11
$0053	$30FF			MOVLW	255
$0054	$00FA			MOVWF	STACK_10
$0055	$0BFB			DECFSZ	STACK_11, F
$0056	$2858			GOTO	$+2
$0057	$285B			GOTO	$+4
$0058	$0BFA			DECFSZ	STACK_10, F
$0059	$2858			GOTO	$-1
$005A	$2855			GOTO	$-5
$005B	$302E			MOVLW	46
$005C	$00FA			MOVWF	STACK_10
$005D	$0BFA			DECFSZ	STACK_10, F
$005E	$285D			GOTO	$-1
;portd7.c,18 :: 		}
$005F	$	L_main_5:
;portd7.c,12 :: 		for(i=0;i<75;i++)
$005F	$0AA0			INCF	_i, 1
$0060	$1903			BTFSC	STATUS, Z
$0061	$0AA1			INCF	_i+1, 1
;portd7.c,18 :: 		}
$0062	$2838			GOTO	L_main_3
$0063	$	L_main_4:
;portd7.c,19 :: 		for(i=0;i<75;i++)
$0063	$01A0			CLRF	_i
$0064	$01A1			CLRF	_i+1
$0065	$	L_main_6:
$0065	$3000			MOVLW	0
$0066	$0221			SUBWF	_i+1, 0
$0067	$1D03			BTFSS	STATUS, Z
$0068	$286B			GOTO	L_main_11
$0069	$304B			MOVLW	75
$006A	$0220			SUBWF	_i, 0
$006B	$	L_main_11:
$006B	$1803			BTFSC	STATUS, C
$006C	$2890			GOTO	L_main_7
;portd7.c,21 :: 		PORTB.F4=1;        //SERVO_2
$006D	$1606			BSF	PORTB, 4
;portd7.c,22 :: 		delay_us(1000);
$006E	$3002			MOVLW	2
$006F	$00FB			MOVWF	STACK_11
$0070	$30FF			MOVLW	255
$0071	$00FA			MOVWF	STACK_10
$0072	$0BFB			DECFSZ	STACK_11, F
$0073	$2875			GOTO	$+2
$0074	$2878			GOTO	$+4
$0075	$0BFA			DECFSZ	STACK_10, F
$0076	$2875			GOTO	$-1
$0077	$2872			GOTO	$-5
$0078	$304A			MOVLW	74
$0079	$00FA			MOVWF	STACK_10
$007A	$0BFA			DECFSZ	STACK_10, F
$007B	$287A			GOTO	$-1
;portd7.c,23 :: 		PORTB.F4=0;
$007C	$1206			BCF	PORTB, 4
;portd7.c,24 :: 		delay_us(19000);
$007D	$3019			MOVLW	25
$007E	$00FB			MOVWF	STACK_11
$007F	$30FF			MOVLW	255
$0080	$00FA			MOVWF	STACK_10
$0081	$0BFB			DECFSZ	STACK_11, F
$0082	$2884			GOTO	$+2
$0083	$2887			GOTO	$+4
$0084	$0BFA			DECFSZ	STACK_10, F
$0085	$2884			GOTO	$-1
$0086	$2881			GOTO	$-5
$0087	$309B			MOVLW	155
$0088	$00FA			MOVWF	STACK_10
$0089	$0BFA			DECFSZ	STACK_10, F
$008A	$2889			GOTO	$-1
$008B	$0000			NOP
;portd7.c,25 :: 		}
$008C	$	L_main_8:
;portd7.c,19 :: 		for(i=0;i<75;i++)
$008C	$0AA0			INCF	_i, 1
$008D	$1903			BTFSC	STATUS, Z
$008E	$0AA1			INCF	_i+1, 1
;portd7.c,25 :: 		}
$008F	$2865			GOTO	L_main_6
$0090	$	L_main_7:
;portd7.c,26 :: 		}
$0090	$2890			GOTO	$
